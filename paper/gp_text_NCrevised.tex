...
state values. The Expectation Maximization (EM) algorithm is widely applied \rev{to infer model parameters} when hidden variables
...
of toys. A nursery easily has a hundred different toys and any subset of these hundred toys may appear in any image. For a hundred toys there is therefore a combinatorics of $2^{100}$ different combinations of toys that can make up an image. An inference task may now be to infer for any given image the toys it contains. If we approached this task using a probabilistic graphical model, we would define a basic such model using a set of one hunderd hidden variables (one for each toy). Given a specific image, inference would then take the form of computing the posterior probability for any combination of toys, and from this the most like combination of toys or the probabilities for the different toys can be computed. If done exactly, this process needed to evaluate all the $2^{100}$ different toy combinations which easily exceeds currently available computational resource. }

Expectation truncation (ET) \citep{LuckeEggert2010} is an approximate EM algorithm for accelerating inference and learning 
in graphical models. Its basic idea is to restrict the inference performed during the E-step
...
\rev{The definition of appropriate selection functions for basic graphical models such as the one of the nursery floor example is already non-trivial. For models incorporating more and more detailed data properties, the definition of selections functions becomes still more demanding, however. For models that also capture mutual object occlusions \citep{HennigesEtAl2014} and/or the position (e.g., of toys) \citep{DaiLucke2014},} the design of suitable selection functions is extremely challenging: it requires both expert knowledge
...
as long as the \emph{relative importance} of the states \rev{likely to contribute posterior probability mass} is preserved.
...
Initialize all latent variables expectations hs (n) i q are initialized randomly
...
In order to demonstrate our method \rev{of autonomously learned selection functions}, we consider three of these
 sparse generative models, and perform inference with our GP-select approach instead of a hand-crafted selection function. The models are:
%
%\begin{enumerate}
\begin{description}
