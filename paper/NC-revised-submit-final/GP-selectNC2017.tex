...
State configurations not in this space (of variables not identified to be relevant) are fixed to $0$ (assigned zero probability mass under the selection function).
%
...
$\vec{y}^{(n)}$,
and \rev{$\mathbb{I}(\vec{s}\in\mathcal{K}_n)=1$ if $\vec{s}\in\mathca4l{K}_n$ is true, and $0$ otherwise. Furthermore, $b$ is a term that does not depend on $s^(n).}
...
iterations. 
\rev{\rev{The optimization of $q_{(n)}$ in early iterations of EM starts from randomly initialized $\vec{s}_h$. Thus it is important that selection-based EM does not "get stuck" assigning higher probability values to $s_h$ selected due to an initially high probability value and therefore always assigned zero value under the selection function.
%deemed relevant. 
To avoid this, we randomly select a few extra hidden indices $H' + \frac{H'}{10}$ to give the algorithm an opportunity to evaluate possibly unused variables which might be relevant for $\vec{y}^{(n)}$.}}
%
