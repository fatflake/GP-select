...
to not select indices that were previously not selected. Thus in order for selection-based EM to not ``get stuck'', it is important select a few extra hidden indices randomly, to give the algorithm an opportunity to evaluate possibly unused variables which might be relevant for $\vec{y}^{(n)}$.}}
%
