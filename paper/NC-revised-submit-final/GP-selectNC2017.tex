...
A \textit{selection function} can be used to identify a subset of size $H'$ ($H' \ll H$)
of salient variables, which in turn is used to define a subset $\mathcal{K}_n \subseteq \{0,1\}^H$ of the possible state configurations of the latent variables $\mathbf{s}^{(n)}$ for each data point. 
