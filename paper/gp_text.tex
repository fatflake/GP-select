...
GP approximations have been used in liu of solving complex partial differential equations \citep{SacksEtal1989, CurrinEtall1991}, to learn data-driven kernel functions for recommendation systems \citep{SchwaighoferEtAl2005}, and recently for quantum chemistry \citep{RuppEtAl2012}. 
...
This work follows the same high level philosophy in that we use GPs to approximate complex/intractable probabilistic models. None of the cited prior work address our problem setting, namely the selection of relevant latent variables by learning a nonparametric relevance function, for use in expectation truncation (ET). %We will include the above discussion to provide context for our work in an updated introduction.
%
...
% FIXME TODO wtf, isn't this... opposite to what we want to say?
...
$\vec{f}(\vec{y}^{(n)}) = (f_1(\vec{y}^{(n)}), \dots, f_H(\vec{y}^{(n)}))$, we define 
 $\gamma\,(\hat{\vec{p}}^{(n)})$ to simultaneously sort the indices of the latent variables in descending order and \textit{reduce} the sorted set to the $H'$ highest (most relevant) variables' indices. % XXX changed rom rebuttal. states' indices. %, (where $H' \ll H$.
...
Third column: Converged dictionary elements $W$ learned by the GP-select with $H'=5$ using the kernel with best performance (matching that of inference with hand-crafted selection function).
In all cases, the model using the GP-select function converged on the ground-truth solution, as the hand-crafted selection functions did.
}\label{fig:sparse}
\end{center}
\end{figure}
...
The training data used for GP regression was $\mathcal{D} = \{ (\vec{y}^{(n)}, \langle \vec{r}\rangle_{q_{n}}^{(n)}) | n = 1, \dots, N \}$, where the targets were the expected cluster responsibilities (posterior probability distribution for each cluster) for all data points, $\langle \vec{r} \rangle$.
...
Although an initially challenging computation, this is not the case given that GP models scale linearly with output dimensionality.
...
Although there are additional computations necessary for an automatic selection function like GP-select, namely due to the adjustment of its parameters,  %Unfortunately, the computation of updating the parameters of a GP regression model is relatively expensive. 
there are many options to reduce computational costs and account for this factor. %reducing the cost of the parameter fitting. 
...
\begin{figure*}[t!]%[ht]
\centering
\includegraphics[width=.5\textwidth]{inveca/dataSamples.png}%{figs/inveca/invec_mod.pdf}
\caption{COIL Dataset \citep{coil100}: 
A handful of data points used in experiments with the Translation Invariant Occlusive (InvECA) model.
}
\label{fig:inveca-data}
\end{figure*}
%
We apply our GP-selection function to the \emph{Invariant Exclusive Component Analysis (InvECA) model}~\citep{DaiEtAl2013}.
...
We compare the visual components learned by using \emph{four different selection functions in the InvECA model}: the hand-crafted selection function used in the original work by ~\citep{DaiLucke2014}, GP-select updated every iteration, GP-select updated every $T*=5$ iterations, and GP-select with incomplete Cholesky decomposition updated every iteration, or $T*=1$ (in this manner we isolate the improvements due to Cholesky from those due to infrequent updates). 
...
\begin{figure*}[t!]%[ht]
\centering
\includegraphics[width=\textwidth]{inveca/params_j.pdf}%{figs/inveca/invec_mod.pdf}
...
\end{figure*}
%
All four versions of the InvECA model using each of the selection functions considered successfully recover each individual objects in the COIL image set. 
...
All bars of the selection functions show equivalent accuracy for the various distances. 
}
\label{fig:inveca1}
%\end{center}
\end{figure}
...
applied the selection functions (for the GP-select selection functions, this was using the final function learned after all EM iterations) to the entire image dataset again, 
...
\ContinuedFloat
\centering
\caption{After about 40 EM iterations, all four selection function versions of the algorithm converge on the same likelihood. Both figures show equivalent performance of the GP-select approaches with the hand-crafted function, but 
...
%
we computed and compared the pseudo log likelihood \cite{DaiEtAl2013} of the original selection function to the three GP-select based ones.
% 
The pseudo log likelihood \cite{DaiEtAl2013} for all selection functions are shown in Figure~\ref{fig:inveca2}.
...
the GP-select selection functions yield no loss in inference performance in comparison to the original hand-engineered selection function. 
Even those using speed-considerate approximations (incomplete Cholesky decomposition of the kernel matrix (GP IChol) and updating kernel hyperparameters only every 5 EM iterations (GP every5)) have the same prediction accuracy on the task.
...
~\citep{sparseGP}, stochastic variational inference \citep{HensmanEtAl2013,Hensman2012}, using parallelization techniques and GPU acceleration \citep{DaiEtAl2014}, or in combination with stochastic gradient descent~\cite{Bottou08thetradeoffs}. 
%XXX new rebuttals:
