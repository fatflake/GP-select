...
This subspace reduction can lead to great a reduction in computation with no loss of accuracy.
In previous work, functions to select states of high posterior mass were 
derived individually for each model of interest, e.g. by taking upper bounds or noiseless limits. 
...
This property is observed to hold in many visual data, auditory data, and general pattern recognition settings.

For more complex graphical models, notably the hierarchical model of \citet{DaiLucke2014},  the design of suitable selection functions
...
Our approach is the first to make ET a general purpose algorithm for discrete latents% using GPs to approximate intractable computations
...
we have applied GP-select in a number of experimental settings.
...
for a recently published hierarchical model for  translation invariant occlusive components analysis
\citep{DaiLucke2014}.
...
only a subset of the $H$ latent variables $s_h^{(n)}$ will be relevant. However, you can have concentrated posteriors even if all latents are relevant.

A \textit{selection function} can be used to identify the subset
...
The variable selection parameter H' is selected based on compute resources available: i.e. as large as resources allow in order to be closer to true EM, although empirically it's been shown that much smaller values suffice (see e.g. \citep{SheikhEtAl2014} App. B on complexity-accuracy trade-offs).
...
\label{eq:affinity}
% remove vector p^:
f_h(\vec{y}^{(n)}) = \hat{p_h}^{(n)} \approx p^{(n)}_h \equiv p(s^{(n)}_h = 1|\vec{y}^{(n)}, \Theta),
\end{equation}
%
...
%3. puke out reduced set of latent states $\Kn$ to use to calculate posterior distribution in E-step\\
    compute truncated posterior $q_{n,t}(\vec{s}^{(n)})$, E-step: \eqref{eq:sel-post}\\ % on reduced set $\Kn$
...
%this matches well with our expectations for the model, and again reveals that it may be essential to learn a nonlinear mapping.
%
Finally, we apply our approach to a recently published hierarchical model for translation invariant occlusion components analysis
\citep{DaiLucke2014}.
...
See the book by \citet{RasmussenGPbook} (Chapter 4, Secton 4.2.4) for a discussion on making new kernels.
...
The training data used for GP regression was $\mathcal{D} = \{ (\vec{y}^{(n)}, \langle s_h^{(n)}\rangle_{q_n}) | n = 1, \dots, N \}$, where the targets were the expected cluster responsibilities (posterior probability distribution for each cluster) for all data points, $\langle s_h\rangle_{q}$.
...
We compare the visual components learned by using \emph{four different selection functions in the InvECA model}: the hand-crafted selection function used in the original work by ~\citet{DaiLucke2014}, GP-select updated every iteration, GP-select updated every $T*=5$ iterations, and GP-select with incomplete Cholesky decomposition updated every iteration, or $T*=1$ (in this manner we isolate the improvements due to Cholesky from those due to infrequent updates). 
...
The number of objects to be learned is $H=20$ and there were $H'=5$ preselected objects.
