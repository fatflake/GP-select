...
that, for these variables, the marginal posterior probability $p^{(n)}_h \equiv p(s^{(n)}_h = 1|\vec{y}^{(n)}, \Theta)$
...
% TODO 
\comm{want some reference to a graphical nmodel here?}
...
which are then used to approximate the posterior distribution by computing the true posterior on the truncated space $q_n(\vec{s})$~\eqref{eq:sel-post}.
...
\comm{expectations suddenly being used - need to mention earlier?}
...
% TODO
As hyperparams of kernels are learned, the composition kernel (4.) can adapt itself to the data and only use the kernel components required. 
...
% ------ FIXME -------- leave this??
\comm{remove this? bit wishy washy }
