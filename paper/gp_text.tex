...
%3. puke out reduced set of latent states $\Kn$ to use to calculate posterior distribution in E-step\\
    compute truncated posterior $q_{n,t}(\vec{s})$, E-step: Eq.~\eqref{eq:sel-post}\\ % on reduced set $\Kn$
    store $\langle \vec{s} \rangle^{(n)}_{q_{t}(\vec{s})}$ for $\vec{p}^{(n)}$ in EM iteration ${t+1}$\\
    update model parameters in M-step\\ 
    }\ENDFOR\\
    update kernel hyperparams every $T^{*}$ EM iterations
}\ENDFOR
\end{algorithmic}
\end{algorithm}
...
\end{equation}
%
where $C$ is the number of mixture components, the task is assign each data point to it's latent cluster.
...
In these experiments we consider three scenarios for EM learning of the data likelihood in Equation~\eqref{eq:mog}: GP-select with an RBF covariance kernel and a linear covariance kernel.%, and the composition kernel used in the sparse coding experiments (RBF, linear, bias, and white).
%
...
This hand-engineered system is costly: the selection function has parameters which need to be hand-tuned, e.g., the number of representative features and %, which leads to problems when this number is either too big or too small.
...
The additional price we have to pay for an automatic selection function like GP-select is the computation of adjusting its parameters. %Unfortunately, the computation of updating the parameters of a GP regression model is relatively expensive. 
