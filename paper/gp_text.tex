...
\comm{More general grounding of motivation, before get into more specific general grounding?  I.e. challenges of big data / scaling and other shiny stuff people love to hear, and how the simplest quick approach that can handle inference should be chosen, stuff from Bottou paper~\cite{Bottou08thetradeoffs}? }
...
We show that GP-selection can produce results with equal performance to the respective manually-derived selection functions.
Interestingly,
...
by the $H \times N$ matrix $\vec{S} = (\vec{s}^{(1)}, \dots, \vec{s}^{(N)})\in \{0,1\}^{N \times H}$ 
...
\comm{do we want to make this an actual equation to make it easier to locate/refer to?}
...
Note that $\Kn$ is not an arbitrary set of the possible states, but is axis aligned according to the latent states that are zero, which is more challenging. \comm{Jan, what did you mean here?}
%FIXME 
...
e.g.\ we try to learn $f$ such that $f_h(\vec{y}^{(n)}) \approx p^{(n)}_h \equiv p(s^{(n)}_h = 1|\vec{y}^{(n)}, \Theta)$.
%
% TODO FIXME
...
\end{equation}
%
% MOVED PART ABOUT SELECTION TO NEXT SEC XXX
...
With the truncated posterior $q(\vec{s})$, inference can be performed using any standard method, such as exact inference or e.g. Gibbs sampling from $q(\vec{s})$ for a further approximation. % sampling approximation of the truncated posterior.
...
to approximate $p_h^{(n)}$ for the entire $N\times H$ set of latent variables $\vec{S}$.
%the marginal posterior probability

% DO NOT EVEN USE: and $\sigma_{noise}^2$ is the degree of Gaussian noise on the function $f_h$.
Thus, prior to each E-step in each EM iteration, within each calculation of the selection function, we calculate the affinity by using a GP to regress $\vec{S}$ onto $\vec{Y}$.  
\comm{expectations suddenly being used}
...
%
\comm{Fix equation! notation for $[K]_{ii}$ somehow less ugly here?} 
\vspace{-.2cm}  % (*1)
...
%
For this experiment, the GP model parameters are optimized at the end of each EM iteration, using a maximum of $20$ gradient updates with L-BFGS \comm{put at least whole name here if not def}.
